...
public class ImageService {
	...
	public List<String> getFileNames() throws IOException {
		...
		final DirectoryStream.Filter<java.nio.file.Path> filter = new DirectoryStream.Filter<java.nio.file.Path>() {
			@Override
			public boolean accept(final java.nio.file.Path entry) throws IOException {
				final boolean sizeOk = Files.size(entry) <= 1024 * 1024 * MAX_SIZE_IN_MB;
				final boolean extensionOk = entry.getFileName().toString().endsWith("jpg") || entry.getFileName().toString().endsWith("png");
		...
		for (final java.nio.file.Path entry : Files.newDirectoryStream(BASE_DIR, filter)) {
	...
	public Response uploadImage(final InputStream in, @HeaderParam("Content-Type") final String fileType, @HeaderParam("Content-Length") final long fileSize) throws IOException {
	...
	public InputStream getJpegImage(@PathParam("name") String fileName) throws IOException {
		...
		fileName += ".jpg";
		final java.nio.file.Path dest = BASE_DIR.resolve(fileName);
